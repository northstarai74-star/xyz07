...
import { deliveryFee } from "@/lib/orders";
...
function CartPage() {
  ...
  const shipping = deliveryFee(subtotal, "standard");
    ...
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_320px]">
        <ul className="space-y-4">
          {lines.map((l) => (
            <li key={l.id} className="flex gap-4 rounded-xl border border-border bg-card p-4">
              {l.image && (
                <img src={l.image} alt={l.name} loading="lazy" className="size-24 rounded-md bg-secondary object-cover" />
              )}
        ...
        <aside className="h-fit rounded-xl border border-border bg-card p-6">
          ...
          <Link
            to="/checkout"
            className="mt-6 block w-full rounded-md bg-primary px-6 py-3 text-center text-sm font-semibold text-primary-foreground hover:opacity-90"
          >
            Proceed to checkout
          </Link>
