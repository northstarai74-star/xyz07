...
function ProductPage() {
  ...
  // `photo` is the clean colour shot, also used as the cart thumbnail. When the
  // selected size has its own photo (back view and close-ups), show that instead.
  const photo = product.images?.[product.colors.indexOf(color)] ?? product.images?.[0];
  const sizePhoto = product.sizeImages?.[color]?.[size];
  const image = sizePhoto ?? photo;
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      ...
      <div className={image ? "grid gap-10 lg:grid-cols-2" : "max-w-2xl"}>
        {image && (
          <figure className="h-fit">
            <div className="overflow-hidden rounded-xl border border-border bg-secondary">
              <img
                ...
                alt={
                  sizePhoto
                    ? `${product.name} in ${color}, size ${size}: front, back view and close-ups`
                    : `${product.name} in ${color}`
                }
            ...
            {sizePhoto && (
              <figcaption className="mt-2 text-xs text-muted-foreground">
                Showing size {size}, with back view and close-up details.
              </figcaption>
            )}
          </figure>
        )}
